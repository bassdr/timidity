...
1. **SF2 modulator support** — PMOD/IMOD chunks are parsed in `sffile.c`, and `sndfont.c` resolves modulators through the full SF2 layer hierarchy (defaults → instrument global → instrument zone → preset additions) into existing Sample fields (`vel_to_fc`, `key_to_fc`, `vel_to_resonance`, `vel_to_atten`, `envelope_velf[]`, `modenv_velf[]`). Velocity→initialAttenuation uses the proper SF2 concave curve via `sf2_vel_cb_table[]` and `cb_to_amp_table[]` at runtime (`playmidi.c:sf2_velocity_amp`). Key and velocity modulator amounts are divided by 127 at storage time to convert from SF2 full-range (source 0-1 * amount) to TiMidity's per-unit runtime formulas. Envelope velocity-follow bpo is set to 127 for SF2 instruments to match the negative-unipolar source semantics. Filter pointer bug fixed (`voice[v].sample` not `&voice[v].sample`). Tested with both FluidR3_GM.sf2 and MuseScore_General.sf2. Only 4 instruments have concave-curve modulators that aren't yet handled — negligible impact.
