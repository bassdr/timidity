...
struct ringbuf {
	char *buf;
	uint32 size;        /* total size in bytes, always power of 2 */
	uint32 mask;        /* size - 1, for fast modulo */
	int64 rdptr, wrptr; /* byte offsets (unbounded) */
};

/* Round up to the next power of 2 (no-op if already power of 2). */
static uint32 next_power_of_2(uint32 v) {
	v--;
	v |= v >> 1;
	v |= v >> 2;
	v |= v >> 4;
...
static void ringbuf_init(struct ringbuf *rb, uint32 size) {
...
static inline int32 ringbuf_available(struct ringbuf *rb) {
	int64 avail = rb->wrptr - rb->rdptr;
	assert(avail >= 0);
	assert(avail <= rb->size);
	return (int32)avail;
}

static inline int32 ringbuf_empty(struct ringbuf *rb) {
	int64 empty = rb->size - ringbuf_available(rb);
	assert(empty >= 0);
	assert(empty <= rb->size);
	return (int32)empty;
}
...
static int32 ringbuf_read(struct ringbuf *rb, char *dst, int64 bytes) {
	int32 avail = ringbuf_available(rb);

	if (bytes > (int64)avail)
		bytes = avail;
	if (bytes <= 0)
		return 0;

	uint32 off = rb->rdptr & rb->mask;
	int64 chunk = (int64)rb->size - (int64)off;
	if (chunk > (int64)bytes)
		chunk = bytes;
	memcpy(dst, rb->buf + off, chunk);
	if (chunk < (int64)bytes)
		memcpy(dst + chunk, rb->buf, (size_t)bytes - (size_t)chunk);
	rb->rdptr += bytes;

	assert(bytes >= INT32_MIN);
	assert(bytes <= INT32_MAX);
	return (int32)bytes;
}

/* Write up to 'bytes' into ring buffer from src. Returns bytes written. */
static int32 ringbuf_write(struct ringbuf *rb, const char *src, int64 bytes) {
	int32 space = ringbuf_empty(rb);

	if (bytes > (int64)space)
		bytes = space;
	if (bytes <= 0)
		return 0;

	int64 off = rb->wrptr & rb->mask;
	int64 chunk = (int64)rb->size - off;
	if (chunk > bytes)
		chunk = bytes;
	memcpy(rb->buf + off, src, (size_t)(chunk > 0 ? chunk : 0));
	if (chunk < bytes)
		memcpy(rb->buf, src + chunk, (size_t)bytes - (size_t)chunk);
	rb->wrptr += bytes;
...
struct pw_ctx {
	...
	int channels;
	uint32 sample_size; /* bytes per frame (channels * bytes_per_sample) */
	uint32 frag_size;   /* fragment size in frames */
	uint32 frags;       /* number of fragments */
	...
	uint32 idle_frames; /* consecutive empty frames from process cb */
	int idle_threshold; /* frames of silence before corking */

	struct ringbuf rb;
	int64 samples_played; /* frames consumed by PipeWire */
...
static void on_process(void *userdata) {
	...
	if (pthread_mutex_trylock(&c->lock) != 0) {
		memset(dst, 0, n_bytes > 0 ? (size_t)n_bytes : 0);
		goto fill_done;
	}

	int32 avail = ringbuf_available(&c->rb);
	...
	if (avail >= n_bytes) {
		...
		int32 got = ringbuf_read(&c->rb, dst, avail);
		 ...
		 */
		uint32 copy = (n_bytes <= (int64)c->last_chunk_size) ? (uint32)n_bytes : c->last_chunk_size;
		...
		for (i = 0; i < n_frames; i++) {
			double gain = 1.0 - (double)i / (double)n_frames;
			int ch;
			for (ch = 0; ch < c->channels; ch++) {
				int off = (i * c->channels + ch) * bps;
				if (bps == 2) {
					int16_t *s = (int16_t *)(dst + off);
					*s = (int16_t)((double)*s * gain);
				} else if (bps == 4) {
...
static int open_output(void) {
	...
	{
		uint32 rb_frames = ctx.frag_size * ctx.frags;
	 ...
	 */
	{
		char latency_str[64];
		int logged = 0;
		(void)snprintf(latency_str, sizeof(latency_str), "%d/%d",
		               ctx.frag_size, dpm.rate);
	...
	if (ctx.wake_fd < 0) {
		ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
		          "PipeWire: cannot subscribe to wake event");
		return -1;
	}
	 ...
	 */
	(void)signal(SIGINT, pw_sigterm_exit);
	(void)signal(SIGTERM, pw_sigterm_exit);
...
static int output_data(char *buf, int32 bytes) {
	...
	while (bytes > 0) {
		...
		/* drain eventfd in case signal woke us instead of condvar */
		uint64_t ignored;
		(void)read(ctx.wake_fd, &ignored, sizeof(ignored));

		if (!__atomic_load_n(&ctx.running, __ATOMIC_ACQUIRE))
			return -1;

		pthread_mutex_lock(&ctx.lock);
		int32 written = ringbuf_write(&ctx.rb, buf, bytes);
		pthread_mutex_unlock(&ctx.lock);
...
static int acntl(int request, void *arg) {
	...
	case PM_REQ_GETFRAGSIZ:
		if (ctx.frag_size == 0)
			return -1;
		*((int *)arg) = (int)(ctx.frag_size * ctx.sample_size);
		return 0;

	case PM_REQ_GETQSIZ:
		*((int *)arg) = (int)(ctx.frag_size * ctx.frags * ctx.sample_size);
		return 0;

	case PM_REQ_GETFILLABLE:
		pthread_mutex_lock(&ctx.lock);
		*((int *)arg) = (int)(ringbuf_empty(&ctx.rb) / ctx.sample_size);
		pthread_mutex_unlock(&ctx.lock);
		return 0;

	case PM_REQ_GETFILLED:
		pthread_mutex_lock(&ctx.lock);
		*((int *)arg) = (int)(ringbuf_available(&ctx.rb) / ctx.sample_size);
		pthread_mutex_unlock(&ctx.lock);
		return 0;

	case PM_REQ_GETSAMPLES:
		pthread_mutex_lock(&ctx.lock);
		*((int *)arg) = (int)(ctx.samples_played > INT_MAX ? INT_MAX : ctx.samples_played);
		pthread_mutex_unlock(&ctx.lock);
		return 0;
	...
	case PM_REQ_OUTPUT_READY:
		 ...
		 */
		{
			int64 frag_bytes = (int64)ctx.frag_size * ctx.sample_size;
			struct timespec ts;
