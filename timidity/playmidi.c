...
#define IS_SYSEX_EVENT_TYPE(event)                                             \
	((event)->type == ME_NONE || (event)->type >= ME_RANDOM_PAN ||         \
	 (event)->b == SYSEX_TAG)
	...
	if (opt_overlap_voice_allow) {
		...
		if (vidq_head[i] == vidq_tail[i]) {
			ctl->cmsg(CMSG_WARNING, VERB_DEBUG_SILLY,
			          "channel=%d, note=%d: Voice is already OFF",
			          ch, note);
			return -1;
		}
...
static void reset_controllers(int c) {
	...
	if (prescanning_flag) {
		if (channel[c].volume >
		    mainvolume_max) { /* pick maximum value of mainvolume */
			mainvolume_max = channel[c].volume;
			ctl->cmsg(CMSG_INFO, VERB_DEBUG,
			          "ME_MAINVOLUME/max (CH:%d VAL:%#x)", c,
			          mainvolume_max);
		}
	}
...
static void redraw_controllers(int c) {
	...
	ctl_mode_event(CTLE_MUTE, 1, c,
	               (IS_SET_CHANNELMASK(channel_mute, c)) ? 1 : 0);
	...
	for (i = 0; i < MAX_CHANNELS; i++) {
		...
		if (ISDRUMCHANNEL(i)) {
			...
			if (special_tonebank >= 0)
				channel[i].bank = (uint8)special_tonebank;
			else
				channel[i].bank = (uint8)default_tonebank;
		}
...
void recompute_freq(int v) {
	...
	if (voice[v].vibrato_control_ratio || channel[ch].mod.val > 0) {
		...
		if (opt_channel_pressure || opt_modulation_wheel) {
			...
			vp->vibrato_depth +=
			    get_midi_controller_pitch_depth(
			        &(channel[ch].mod)) +
			    get_midi_controller_pitch_depth(
			        &(channel[ch].bend)) +
			    get_midi_controller_pitch_depth(
		...
		if (channel[ch].mod.val > 0) {
			if (vp->vibrato_control_ratio == 0) {
				vp->vibrato_control_ratio =
				    vp->orig_vibrato_control_ratio =
				        (int)(cnv_Hz_to_vib_ratio(5.0) *
	...
	/* At least for GM2, it's recommended not to apply master_tuning for
	 * drum channels */
	...
	tuning += (channel[ch].rpnmap[RPN_ADDR_0001] - 0x40 +
	           (channel[ch].rpnmap[RPN_ADDR_0002] - 0x40) * 64)
	          << 7;
	...
	if (ISDRUMCHANNEL(ch) && channel[ch].drums[note] != NULL &&
		...
		tuning += (channel[ch].drums[note]->fine +
		           channel[ch].drums[note]->coarse * 64)
		          << 7;
	}
	...
	if (opt_modulation_envelope) {
		if (voice[v].sample->tremolo_to_pitch) {
			tuning +=
			    lookup_triangular(voice[v].tremolo_phase >>
			                      RATE_SHIFT) *
			        (voice[v].sample->tremolo_to_pitch << 13) /
			        100.0 +
			    0.5;
		...
		if (voice[v].sample->modenv_to_pitch) {
			tuning += voice[v].last_modenv_volume *
			              (voice[v].sample->modenv_to_pitch << 13) /
			              100.0 +
			          0.5;
			channel[ch].pitchfactor = 0;
		}
	...
	if (!opt_pure_intonation && opt_temper_control &&
		...
		case 1:
			if (current_temper_keysig < 8)
				f = freq_table_pytha[current_temper_freq_table]
				                    [note];
			else
				f = freq_table_pytha[current_temper_freq_table +
				                     12][note];
	...
	a = TIM_FSCALE(
	        ((double)voice[v].sample->sample_rate *
	...
	if (voice[v].sample_increment == 0) {
		fprintf(
		    stderr, "Invalid sample increment a=%e %ld %ld %ld %ld%s\n",
...
static uint8 calc_velocity(int32 ch, int32 vel) {
	int32 velocity;
	velocity = channel[ch].velocity_sense_depth * vel / 64 +
	           (channel[ch].velocity_sense_offset - 64) * 2;
	if (velocity > 127)
		return 127;
	if (velocity < 0)
		return 0;
...
static FLOAT_T sf2_velocity_amp(uint8 velocity, int16 vel_to_atten) {
	...
	atten_cb =
	    fclamp(sf2_vel_cb_table[velocity] * vel_to_atten / 960., 0., 960.);
...
static void recompute_amp(int v) {
	...
	if (opt_user_volume_curve) {
		tempamp = master_volume * voice[v].sample->volume *
	...
	} else if (play_system_mode ==
	           GS_SYSTEM_MODE) { /* use measured curve */
		tempamp = master_volume * voice[v].sample->volume *
		...
		tempamp =
		    master_volume * voice[v].sample->volume *
	...
	if (ISDRUMCHANNEL(ch)) {
		...
		tempamp *=
		    (double)opt_drum_power * 0.01f; /* global drum power */
	}
	...
	if (!(play_mode->encoding & PE_MONO)) {
		if (voice[v].panning == 64) {
			...
			voice[v].left_amp = voice[v].right_amp =
			    TIM_FSCALENEG(tempamp * pan_table[64], 27);
			...
			voice[v].left_amp = TIM_FSCALENEG(
			    tempamp * pan_table[128 - voice[v].panning], 27);
			voice[v].right_amp = TIM_FSCALENEG(
			    tempamp * pan_table[voice[v].panning], 27);
		}
	} else {
		voice[v].panned = PANNED_CENTER;
...
void recompute_channel_filter(int ch, int note) {
	...
	if (channel[ch].soft_pedal != 0) {
		if (note > 49) { /* tre corde */
			coef *= 1.0 - 0.20 * ((double)channel[ch].soft_pedal) /
			                  127.0f;
		} else { /* una corda (due corde) */
			coef *= 1.0 - 0.25 * ((double)channel[ch].soft_pedal) /
			                  127.0f;
		}
	}
	...
	if (!ISDRUMCHANNEL(ch)) {
		/* NRPN Filter Cutoff */
		coef *=
		    pow(1.26, (double)(channel[ch].param_cutoff_freq) / 8.0f);
		/* NRPN Resonance */
...
void init_voice_filter(int i) {
	...
	if (opt_lpf_def && voice[i].sample->cutoff_freq) {
		...
		voice[i].fc.orig_reso_dB =
		    (double)voice[i].sample->resonance / 10.0f - 3.01f;
		...
		if (opt_lpf_def == 2) {
			...
			voice[i].fc.gain = pow(
			    10.0f, -voice[i].fc.orig_reso_dB / 2.0f / 20.0f);
			voice[i].fc.type = 1;
		}
...
void recompute_voice_filter(int v) {
	...
	if (ISDRUMCHANNEL(ch) && channel[ch].drums[note] != NULL) {
		...
		coef *= pow(
		    1.26,
		    (double)(channel[ch].drums[note]->drum_cutoff_freq) / 8.0f);
		/* NRPN Drum Instrument Filter Resonance */
		reso += (double)channel[ch].drums[note]->drum_resonance *
		        RESONANCE_COEFF;
	}
	...
	if (opt_channel_pressure) {
		...
		depth_cent +=
		    get_midi_controller_filter_depth(&(channel[ch].mod)) +
	...
	if (sp->vel_to_fc) { /* velocity to filter cutoff frequency */
		cent +=
		    sp->vel_to_fc *
		    (127. - fmax(voice[v].velocity, sp->vel_to_fc_threshold)) /
		    127.;
	}
	if (sp->vel_to_resonance) { /* velocity to filter resonance */
		reso += (double)voice[v].velocity * sp->vel_to_resonance /
		        127.0f / 10.0f;
	}
	if (sp->key_to_fc) { /* filter cutoff key-follow */
		cent +=
		    sp->key_to_fc * (double)(voice[v].note - sp->key_to_fc_bpo);
	}

	if (opt_modulation_envelope) {
		if (voice[v].sample->tremolo_to_fc || depth_cent != 0) {
			cent += ((double)voice[v].sample->tremolo_to_fc +
			         depth_cent) *
			        lookup_triangular(voice[v].tremolo_phase >>
			                          RATE_SHIFT);
		}
		if (sp->modenv_to_fc ||
		    (sp->vel_to_modenv_to_fc && voice[v].velocity) ||
		    sp->vel_to_modenv_to_fc_cc) {
			/* Total modenv→filterFc = generator amount + velocity
			 * modulators. Linear: vel/127 * amount (positive
			 * unipolar). Concave: (1 - sf2_vel_cb_table[vel]/960) *
			 * amount (positive unipolar). */
			double modenv_fc = (double)sp->modenv_to_fc;
			if (sp->vel_to_modenv_to_fc) {
				modenv_fc += sp->vel_to_modenv_to_fc *
				             (double)voice[v].velocity / 127.0;
			}
			if (sp->vel_to_modenv_to_fc_cc) {
				modenv_fc +=
				    sp->vel_to_modenv_to_fc_cc *
				    (1.0 - sf2_vel_cb_table[voice[v].velocity] /
				               960.0);
			}
	...
	fc->freq =
	    fclamp((double)fc->orig_freq * coef, 5., play_mode->rate / 2.);

	fc->reso_dB =
	    fclamp(fc->orig_reso_dB + channel[ch].resonance_dB + reso, 0., 96.);
	...
	if (fc->type == 1) { /* Chamberlin filter */
		...
		if (fc->reso_dB > fc->orig_reso_dB / 2) {
			fc->gain =
			    pow(10.0f,
			        (fc->reso_dB - fc->orig_reso_dB / 2) / 20.0f);
		}
	}
...
static int32 calc_random_delay(int ch, int note) {
	...
	if (bank->tone[nprog].rnddelay == 0) {
		...
		return (int32)((double)bank->tone[nprog].rnddelay *
		               play_mode->rate / 1000.0 *
		               (get_pink_noise_light(&global_pink_noise_light) +
		                1.0f) *
		               0.5);
	}
}
...
void recompute_bank_parameter(int ch, int note) {
	...
	if (ISDRUMCHANNEL(ch)) {
		...
		if (drum->reverb_level == -1 &&
		    bank->tone[nprog].reverb_send != -1) {
			drum->reverb_level = bank->tone[nprog].reverb_send;
		}
		if (drum->chorus_level == -1 &&
		    bank->tone[nprog].chorus_send != -1) {
			drum->chorus_level = bank->tone[nprog].chorus_send;
		}
		if (drum->delay_level == -1 &&
		    bank->tone[nprog].delay_send != -1) {
...
Instrument *play_midi_load_instrument(int dr, int bk, int prog) {
	...
	/* tone->name is NULL if "soundfont" directive is used, and ip is NULL
	 * when not preloaded */
	/* dr: not sure but only drumsets are concerned at the moment */
	if (dr && !tone->name &&
	    ((ip = tone->instrument) == MAGIC_LOAD_INSTRUMENT || ip == NULL) &&
...
static int reduce_voice(void) {
	...
	for (j = 0; j < i; j++) {
		...
		if (voice[j].status &
		    ~(VOICE_ON | VOICE_DIE | VOICE_SUSTAINED)) {
			/* find lowest volume */
			v = voice[j].left_mix;
...
static int find_free_voice(void) {
	...
	for (i = 0; i < nv; i++) {
		if (voice[i].status & ~(VOICE_ON | VOICE_DIE) &&
		    !(voice[i].sample && voice[i].sample->note_to_use &&
...
static int find_samples(MidiEvent *e, int *vlist) {
	...
	if (channel[ch].special_sample > 0) {
		...
		return select_play_sample(s->sample, s->samples, &note, vlist,
		                          e, (s->type == INST_GUS) ? 1 : 32);
	}
	...
	if (ISDRUMCHANNEL(ch)) {
		...
		/* if (ip->type == INST_GUS && ip->samples != 1)
		        ctl->cmsg(CMSG_WARNING, VERB_VERBOSE,
		                        "Strange: percussion instrument with %d
		   samples!", ip->samples); */
		...
		note = (int)((ip->sample->note_to_use) ? ip->sample->note_to_use
		                                       : e->a) +
	...
	if (!prescanning_flag) {
		...
		for (i = 0; i < nv; i++) {
			...
			if (!opt_realtime_playing && allocate_cache_size > 0 &&
				...
				voice[j].cache =
				    resamp_cache_fetch(voice[j].sample, note);
				if (voice[j].cache) /* cache hit */
					voice[j].sample =
					    voice[j].cache->resampled;
			} else
				voice[j].cache = NULL;
		}
	}
...
static int select_play_sample(Sample *splist, int nsp, int *note, int *vlist,
                              MidiEvent *e, int maxnv) {
	...
	if (ISDRUMCHANNEL(ch))
		...
		if (opt_pure_intonation) {
			if (current_keysig < 8)
				...
				f = freq_table_pureint[current_freq_table + 12]
				                      [*note];
			...
			f = freq_table[*note];
		if (!opt_pure_intonation && opt_temper_control && tt == 0 &&
		    f != freq_table[*note]) {
	...
	if (nv == 0) { /* we must select at least one sample. */
		...
		/* Otherwise, it's a lonely right sample, but better than
		 * nothing. */
		j = vlist[nv] = find_voice(e);
...
static int find_voice(MidiEvent *e) {
	...
	status_check =
	    (opt_overlap_voice_allow) ? (VOICE_OFF | VOICE_SUSTAINED) : 0xff;
	...
	for (i = 0; i < upper_voices; i++)
		if (voice[i].status != VOICE_FREE && voice[i].channel == ch) {
			if (voice[i].note == note &&
			    (voice[i].status & status_check))
				kill_note(i);
			else if (mono_check)
				kill_note(i);
			else if (altassign &&
			         find_altassign(altassign, voice[i].note))
				kill_note(i);
...
FLOAT_T get_note_freq_sf(int16 scale_freq, int16 scale_factor, uint8 note) {
	FLOAT_T f = freq_table[note];
	/* GUS/SF2 - Scale Tuning */
	if (scale_factor != 1024) {
		f *= pow(2.0, (note - scale_freq) * (scale_factor - 1024) / 12288.0);
	}
	return f;
}

FLOAT_T get_note_freq(Sample *sp, uint8 note) {
	return get_note_freq_sf(sp->scale_freq, sp->scale_factor, note);
}
...
static void init_voice_vibrato(int v) {
	...
	nrpn_vib_flag = opt_nrpn_vibrato && (channel[ch].vibrato_ratio != 1.0 ||
	                                     channel[ch].vibrato_depth != 0);
	...
	if (nrpn_vib_flag) {
		if (vp->sample->vibrato_control_ratio == 0) {
			ratio = cnv_Hz_to_vib_ratio(5.0) *
			        channel[ch].vibrato_ratio;
		} else {
			ratio = (double)vp->sample->vibrato_control_ratio *
			        channel[ch].vibrato_ratio;
		}
		if (ratio < 0) {
			ratio = 0;
	...
	if (nrpn_vib_flag) {
		vp->vibrato_depth =
		    vp->sample->vibrato_depth + channel[ch].vibrato_depth;
		if (vp->vibrato_depth > VIBRATO_DEPTH_MAX) {
			vp->vibrato_depth = VIBRATO_DEPTH_MAX;
		} else if (vp->vibrato_depth < 1) {
			vp->vibrato_depth = 1;
	...
	/* vibrato delay */
	vp->vibrato_delay =
	    vp->sample->vibrato_delay + channel[ch].vibrato_delay;
...
static void init_voice_pan_delay(int v) {
	...
	if (opt_pan_delay && channel[ch].insertion_effect == 0 &&
		...
		if (vp->panning == 64) {
			vp->delay +=
			    pan_delay_table[64] * play_mode->rate / 1000;
		} else {
			if (pan_delay_table[vp->panning] >
			    pan_delay_table[127 - vp->panning]) {
				pan_delay_diff =
				    pan_delay_table[vp->panning] -
				    pan_delay_table[127 - vp->panning];
				vp->delay += (pan_delay_table[vp->panning] -
				              pan_delay_diff) *
				             play_mode->rate / 1000;
			} else {
				pan_delay_diff =
			...
			vp->pan_delay_rpt =
			    pan_delay_diff * play_mode->rate / 1000;
		}
...
static void init_voice_portamento(int v) {
	...
	if (channel[ch].porta_control_ratio) {
		if (channel[ch].last_note_fine == -1) {
			...
			vp->porta_control_ratio =
			    channel[ch].porta_control_ratio;
...
static void start_note(MidiEvent *e, int i, int vid, int cnt) {
	...
	if (j == 0 || special_patch[j] == NULL)
		...
		voice[i].sample_offset = special_patch[j]->sample_offset
		                         << FRACTION_BITS;
		if (voice[i].sample->modes & MODES_LOOPING) {
			if (voice[i].sample_offset > voice[i].sample->loop_end)
				voice[i].sample_offset =
				    voice[i].sample->loop_start;
		} else if (voice[i].sample_offset >
		           voice[i].sample->data_length) {
			free_voice(i);
			return;
		}
	}
...
static void finish_note(int i) {
	if (voice[i].sample->modes & MODES_ENVELOPE) {
		...
		if (current_file_info->pcm_mode != PCM_MODE_NON) {
			...
			/* Set status to OFF so resample_voice() will let this
			voice out of its loop, if any. In any case, this voice
			dies when it hits the end of its data
			(ofs>=data_length). */
...
static void set_envelope_time(int ch, int val, int stage) {
	...
	case EG_ATTACK: /* Attack */
		ctl->cmsg(CMSG_INFO, VERB_NOISY, "Attack Time (CH:%d VALUE:%d)",
		          ch, val);
		break;
	case EG_DECAY: /* Decay */
		ctl->cmsg(CMSG_INFO, VERB_NOISY, "Decay Time (CH:%d VALUE:%d)",
		          ch, val);
		break;
	case EG_RELEASE: /* Release */
		ctl->cmsg(CMSG_INFO, VERB_NOISY,
		          "Release Time (CH:%d VALUE:%d)", ch, val);
		break;
	default:
		ctl->cmsg(CMSG_INFO, VERB_NOISY, "? Time (CH:%d VALUE:%d)", ch,
		          val);
	}
...
static void new_delay_voice(int v1, int level) {
	...
	if (ISDRUMCHANNEL(ch) && channel[ch].drums[voice[v1].note] != NULL) {
		level *=
		    (FLOAT_T)channel[ch].drums[voice[v1].note]->delay_level /
		    127.0;
	}

	vol =
	    voice[v1].velocity * level / 127.0 * delay_status_gs.level_ratio_c;
...
static void new_chorus_voice(int v1, int level) {
	...
	if (ISDRUMCHANNEL(ch) && channel[ch].drums[voice[v1].note] != NULL) {
		level *=
		    (FLOAT_T)channel[ch].drums[voice[v1].note]->chorus_level /
		    127.0;
	}
...
static void new_chorus_voice_alternate(int v1, int level) {
	...
	if (frac < 0.5 && frac > 0.40) {
		...
		if (!(play_mode->encoding & PE_MONO))
			delay +=
			    (0.5 - frac) * (1.0 - labs(64 - pan) / 63.0) / freq;
	...
	if (!(play_mode->encoding & PE_MONO) && ISDRUMCHANNEL(ch) &&
	    voice[v1].panned == PANNED_CENTER) {
		int i, j;

		/* force Rimshot (37), Snare1 (38), Snare2 (40), and XG #34 to
		 * have the same delay, otherwise there will be bad voice
		 * cancellation.
		 */
		if (voice[v1].note == 37 || voice[v1].note == 38 ||
		    voice[v1].note == 40 ||
		    (voice[v1].note == 34 &&
		...
		/* force Kick1 (35), Kick2 (36), and XG Kick #33 to have the
		 * same delay, otherwise there will be bad voice cancellation.
		 */
		if (voice[v1].note == 35 || voice[v1].note == 36 ||
		    (voice[v1].note == 33 &&
		     play_system_mode == XG_SYSTEM_MODE)) {
			for (i = 0; i < upper_voices; i++) {
				if (voice[i].status & (VOICE_DIE | VOICE_FREE))
...
static void note_on_prescan(MidiEvent *ev) {
	...
	if (ISDRUMCHANNEL(ch) && channel[ch].drums[note] != NULL &&
	    !get_rx_drum(channel[ch].drums[note],
	                 RX_NOTE_ON)) { /* Rx. Note On */
		return;
	}
...
static void note_on(MidiEvent *e) {
	...
	for (i = 0; i < nv; i++) {
		...
		if (ISDRUMCHANNEL(ch) && channel[ch].drums[note] != NULL &&
			...
			ctl_mode_event(CTLE_PANNING, 1, ch,
			               channel[ch].panning);
		}
		...
		if ((channel[ch].chorus_level || opt_surround_chorus)) {
			if (opt_surround_chorus)
				new_chorus_voice_alternate(
				    v, channel[ch].chorus_level);
			else
...
static void note_off(MidiEvent *e) {
	...
	if (ISDRUMCHANNEL(ch)) {
		...
		if (channel[ch].drums[nprog] != NULL &&
			...
			/* uh oh, this drum doesn't have an instrument loaded
			 * yet */
			if (bank->tone[nprog].instrument == NULL)
				return;

			/* this drum is not loaded for some reason (error
			 * occured?) */
			if (IS_MAGIC_INSTRUMENT(bank->tone[nprog].instrument))
				return;

			/* only disallow Note Off if the drum sample is not
			 * looped */
			if (!(bank->tone[nprog].instrument->sample->modes &
			      MODES_LOOPING))
	...
	for (i = 0; i < uv; i++) {
		if (voice[i].status == VOICE_ON && voice[i].channel == ch &&
		    voice[i].note == note && voice[i].vid == vid) {
			if (sustain) {
				voice[i].status = VOICE_SUSTAINED;
				ctl_note_event(i);
			} else
	...
	if (opt_channel_pressure) {
		...
		for (i = 0; i < uv; i++)
			if (voice[i].status == VOICE_ON &&
			    voice[i].channel == ch && voice[i].note == note) {
				recompute_amp(i);
				apply_envelope_to_amp(i);
				recompute_freq(i);
				recompute_voice_filter(i);
	...
	for (i = 0; i < uv; i++) {
		if ((voice[i].channel == c) &&
...
#endif
			{
				...
				v1 = i; /* base voice */
				v2 = voice[i]
				         .chorus_link; /* sub voice (detuned) */

				if (opt_surround_chorus) /* Surround chorus mode
				                            by Eric. */
				{
					int panlevel;

					if (!pan)
						pan = 1; /* make hard left be 1
						            instead of 0 */
					panlevel = 63;
					if (pan - panlevel < 1)
						panlevel = pan - 1;
					...
					if (pan > 60 &&
					    pan < 68) /* PANNED_CENTER */
						voice[v2].panning =
						    64 + int_rand(40) -
						    20; /* 64 +- rand(20) */
					else if (pan <
					         CHORUS_OPPOSITE_THRESHOLD)
						voice[v2].panning = 127;
					else if (pan >
					         127 -
					             CHORUS_OPPOSITE_THRESHOLD)
...
void play_midi_setup_drums(int ch, int note) {
	channel[ch].drums[note] = (struct DrumParts *)new_segment(
	    &playmidi_pool, sizeof(struct DrumParts));
...
static void set_reverb_level(int ch, int level) {
	if (level == -1) {
		channel[ch].reverb_level = channel[ch].reverb_id =
		    (opt_reverb_control < 0) ? -opt_reverb_control & 0x7f
...
int get_reverb_level(int ch) {
	if (channel[ch].reverb_level == -1)
		return (opt_reverb_control < 0) ? -opt_reverb_control & 0x7f
		                                : DEFAULT_REVERB_SEND_LEVEL;
...
static void make_drum_effect(int ch) {
	...
	if (channel[ch].drum_effect_flag == 0) {
		...
		channel[ch].drum_effect = (struct DrumPartEffect *)safe_malloc(
		    sizeof(struct DrumPartEffect) * num);

		for (i = 0; i < num; i++) {
			de = &(channel[ch].drum_effect[i]);
			de->note = note = note_table[i];
			drum = channel[ch].drums[note];
			de->reverb_send = (int32)drum->reverb_level *
			                  (int32)get_reverb_level(ch) / 127;
			de->chorus_send = (int32)drum->chorus_level *
			                  (int32)channel[ch].chorus_level / 127;
			de->delay_send = (int32)drum->delay_level *
			                 (int32)channel[ch].delay_level / 127;
...
void midi_program_change(int ch, int prog) {
	...
	case GS_SYSTEM_MODE: /* GS */
		...
		case 1:
			channel[ch].mapID =
			    (dr) ? SC_55_DRUM_MAP : SC_55_TONE_MAP;
			break;
		case 2:
			channel[ch].mapID =
			    (dr) ? SC_88_DRUM_MAP : SC_88_TONE_MAP;
			break;
		case 3:
			channel[ch].mapID =
			    (dr) ? SC_88PRO_DRUM_MAP : SC_88PRO_TONE_MAP;
			break;
		case 4:
			channel[ch].mapID =
			    (dr) ? SC_8850_DRUM_MAP : SC_8850_TONE_MAP;
			break;
		default:
			break;
		}
	...
	case XG_SYSTEM_MODE: /* XG */
		...
		case 0: /* Normal */
			 ...
			 *
			 * I don't have the original email from my archived
			 * inbox, but I found a reply I made in my archived
			 * sent-mail from 1999.  A September 5th message to
			 * Masanao Izumo is discussing a problem with a
			 * "reapxg.mid", a file which I still have, and how it
			 * issues an MSB=0 with a program change on ch 9, thus
			 * turning it into a melodic channel.  The strange thing
			 * is, this doesn't happen on XG hardware, nor on the XG
			 * softsynth.  It continues to play as a normal drum.
			 * The author of the midi file obviously intended it to
			 * be drumset 16 too.  The original fix was to detect
			 * LSB == -1, then break so as to not set it to a
			 * melodic channel.  I'm guessing that this somehow got
			 * mutated into checking for 127 instead, and the
			 * current FIXME is related to the original hack from
			 * Sept 1999.  The Sept 5th email discusses patches
			 * being applied to version 2.5.1 to get XG drums to
			 * work properly, and a Sept 7th email to someone else
			 * discusses the fixes being part of the
			 * latest 2.6.0-beta3.  A September 23rd email to
			 * Masanao Izumo specifically mentions the LSB == -1
			 * hack (and reapxg.mid not playing "correctly"
			 * anymore), as well as new changes in 2.6.0 that broke
			 * a lot of other XG files (XG drum support was
			 * extremely buggy in 1999 and we were still trying to
			 * figure out how to initialize things to reproduce
			 * hardware behavior).  An October 5th email says
			 * that 2.5.1 was correct, 2.6.0 had very broken XG drum
			 * changes, and 2.6.1 still has problems.  Further
			 * discussions ensued over what was "correct": to follow
			 * the XG spec, or to reproduce "features" / bugs in the
			 * hardware.  I can't find the rest of the discussions,
			 * but I think it ended with us agreeing to just follow
			 * the spec and not try to reproduce the hardware
			 * strangeness.  I don't know how the current FIXME
			 * wound up the way it is now.  I'm still going to guess
			 * it is related to the old reapxg.mid hack.
			 *
			 * Now that reset_midi() initializes
			 * channel[ch].bank_lsb to 0 instead of -1, checking for
			 * LSB == -1 won't do anything anymore, so changing the
			 * above
			 * FIXME to the original == -1 won't do any good.  It is
			 * best to just #if 0 it out and leave it here as a
			 * reminder that there is at least one XG hardware /
			 * softsynth "bug" that is not reproduced by timidity at
			 * the moment.
			 *
			 * If the current FIXME actually reproduces some other
			 * XG hadware bug that I don't know about, then it may
			 * have a valid purpose.  I just don't know what that
			 * purpose is at the moment.  Perhaps someone else does?
			 * I still have src going back to 2.10.4, and the FIXME
			 * comment was already there by then.  I don't see any
			 * entries in the Changelog that could explain it
			 * either.  If someone has src from 2.5.1 through 2.10.3
			 * and wants to investigate this further, go for it :)
			 */
	...
	if (dr) {
		...
		channel[ch].bank =
		    (special_tonebank >= 0) ? special_tonebank : newbank;
...
static void process_sysex_event(int ev, int ch, int val, int b) {
	...
	if (ev == ME_SYSEX_MSB) {
		...
		channel[ch].sysex_xg_msb_val = val;
	           ...
	           ME_SYSEX_LSB) { /* Universal system exclusive message */
		...
		case 0x00: /* CAf Pitch Control */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf Pitch Control (CH:%d %d semitones)", ch,
			          channel[ch].caf.pitch);
			break;
		case 0x01: /* CAf Filter Cutoff Control */
			channel[ch].caf.cutoff = (val - 64) * 150;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf Filter Cutoff Control (CH:%d %d cents)",
			          ch, channel[ch].caf.cutoff);
			break;
		case 0x02: /* CAf Amplitude Control */
			channel[ch].caf.amp = (float)val / 64.0f - 1.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf Amplitude Control (CH:%d %.2f)", ch,
			          channel[ch].caf.amp);
			break;
		case 0x03: /* CAf LFO1 Rate Control */
			channel[ch].caf.lfo1_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO1 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].caf.lfo1_rate);
			break;
		case 0x04: /* CAf LFO1 Pitch Depth */
			channel[ch].caf.lfo1_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO1 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].caf.lfo1_pitch_depth);
			break;
		case 0x05: /* CAf LFO1 Filter Depth */
			channel[ch].caf.lfo1_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO1 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].caf.lfo1_tvf_depth);
			break;
		case 0x06: /* CAf LFO1 Amplitude Depth */
			channel[ch].caf.lfo1_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO1 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].caf.lfo1_tva_depth);
			break;
		case 0x07: /* CAf LFO2 Rate Control */
			channel[ch].caf.lfo2_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO2 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].caf.lfo2_rate);
			break;
		case 0x08: /* CAf LFO2 Pitch Depth */
			channel[ch].caf.lfo2_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO2 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].caf.lfo2_pitch_depth);
			break;
		case 0x09: /* CAf LFO2 Filter Depth */
			channel[ch].caf.lfo2_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO2 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].caf.lfo2_tvf_depth);
			break;
		case 0x0A: /* CAf LFO2 Amplitude Depth */
			channel[ch].caf.lfo2_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CAf LFO2 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].caf.lfo2_tva_depth);
			break;
		case 0x0B: /* PAf Pitch Control */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf Pitch Control (CH:%d %d semitones)", ch,
			          channel[ch].paf.pitch);
			break;
		case 0x0C: /* PAf Filter Cutoff Control */
			channel[ch].paf.cutoff = (val - 64) * 150;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf Filter Cutoff Control (CH:%d %d cents)",
			          ch, channel[ch].paf.cutoff);
			break;
		case 0x0D: /* PAf Amplitude Control */
			channel[ch].paf.amp = (float)val / 64.0f - 1.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf Amplitude Control (CH:%d %.2f)", ch,
			          channel[ch].paf.amp);
			break;
		case 0x0E: /* PAf LFO1 Rate Control */
			channel[ch].paf.lfo1_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO1 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].paf.lfo1_rate);
			break;
		case 0x0F: /* PAf LFO1 Pitch Depth */
			channel[ch].paf.lfo1_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO1 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].paf.lfo1_pitch_depth);
			break;
		case 0x10: /* PAf LFO1 Filter Depth */
			channel[ch].paf.lfo1_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO1 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].paf.lfo1_tvf_depth);
			break;
		case 0x11: /* PAf LFO1 Amplitude Depth */
			channel[ch].paf.lfo1_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO1 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].paf.lfo1_tva_depth);
			break;
		case 0x12: /* PAf LFO2 Rate Control */
			channel[ch].paf.lfo2_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO2 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].paf.lfo2_rate);
			break;
		case 0x13: /* PAf LFO2 Pitch Depth */
			channel[ch].paf.lfo2_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO2 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].paf.lfo2_pitch_depth);
			break;
		case 0x14: /* PAf LFO2 Filter Depth */
			channel[ch].paf.lfo2_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO2 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].paf.lfo2_tvf_depth);
			break;
		case 0x15: /* PAf LFO2 Amplitude Depth */
			channel[ch].paf.lfo2_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "PAf LFO2 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].paf.lfo2_tva_depth);
			break;
		case 0x16: /* MOD Pitch Control */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD Pitch Control (CH:%d %d semitones)", ch,
			          channel[ch].mod.pitch);
			break;
		case 0x17: /* MOD Filter Cutoff Control */
			channel[ch].mod.cutoff = (val - 64) * 150;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD Filter Cutoff Control (CH:%d %d cents)",
			          ch, channel[ch].mod.cutoff);
			break;
		case 0x18: /* MOD Amplitude Control */
			channel[ch].mod.amp = (float)val / 64.0f - 1.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD Amplitude Control (CH:%d %.2f)", ch,
			          channel[ch].mod.amp);
			break;
		case 0x19: /* MOD LFO1 Rate Control */
			channel[ch].mod.lfo1_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO1 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].mod.lfo1_rate);
			break;
		case 0x1A: /* MOD LFO1 Pitch Depth */
			channel[ch].mod.lfo1_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO1 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].mod.lfo1_pitch_depth);
			break;
		case 0x1B: /* MOD LFO1 Filter Depth */
			channel[ch].mod.lfo1_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO1 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].mod.lfo1_tvf_depth);
			break;
		case 0x1C: /* MOD LFO1 Amplitude Depth */
			channel[ch].mod.lfo1_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO1 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].mod.lfo1_tva_depth);
			break;
		case 0x1D: /* MOD LFO2 Rate Control */
			channel[ch].mod.lfo2_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO2 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].mod.lfo2_rate);
			break;
		case 0x1E: /* MOD LFO2 Pitch Depth */
			channel[ch].mod.lfo2_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO2 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].mod.lfo2_pitch_depth);
			break;
		case 0x1F: /* MOD LFO2 Filter Depth */
			channel[ch].mod.lfo2_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO2 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].mod.lfo2_tvf_depth);
			break;
		case 0x20: /* MOD LFO2 Amplitude Depth */
			channel[ch].mod.lfo2_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "MOD LFO2 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].mod.lfo2_tva_depth);
			break;
		case 0x21: /* BEND Pitch Control */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND Pitch Control (CH:%d %d semitones)", ch,
			          channel[ch].bend.pitch);
			break;
		case 0x22: /* BEND Filter Cutoff Control */
			channel[ch].bend.cutoff = (val - 64) * 150;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND Filter Cutoff Control (CH:%d %d cents)",
			          ch, channel[ch].bend.cutoff);
			break;
		case 0x23: /* BEND Amplitude Control */
			channel[ch].bend.amp = (float)val / 64.0f - 1.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND Amplitude Control (CH:%d %.2f)", ch,
			          channel[ch].bend.amp);
			break;
		case 0x24: /* BEND LFO1 Rate Control */
			channel[ch].bend.lfo1_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO1 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].bend.lfo1_rate);
			break;
		case 0x25: /* BEND LFO1 Pitch Depth */
			channel[ch].bend.lfo1_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO1 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].bend.lfo1_pitch_depth);
			break;
		case 0x26: /* BEND LFO1 Filter Depth */
			channel[ch].bend.lfo1_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO1 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].bend.lfo1_tvf_depth);
			break;
		case 0x27: /* BEND LFO1 Amplitude Depth */
			channel[ch].bend.lfo1_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO1 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].bend.lfo1_tva_depth);
			break;
		case 0x28: /* BEND LFO2 Rate Control */
			channel[ch].bend.lfo2_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO2 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].bend.lfo2_rate);
			break;
		case 0x29: /* BEND LFO2 Pitch Depth */
			channel[ch].bend.lfo2_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO2 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].bend.lfo2_pitch_depth);
			break;
		case 0x2A: /* BEND LFO2 Filter Depth */
			channel[ch].bend.lfo2_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO2 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].bend.lfo2_tvf_depth);
			break;
		case 0x2B: /* BEND LFO2 Amplitude Depth */
			channel[ch].bend.lfo2_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "BEND LFO2 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].bend.lfo2_tva_depth);
			break;
		case 0x2C: /* CC1 Pitch Control */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 Pitch Control (CH:%d %d semitones)", ch,
			          channel[ch].cc1.pitch);
			break;
		case 0x2D: /* CC1 Filter Cutoff Control */
			channel[ch].cc1.cutoff = (val - 64) * 150;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 Filter Cutoff Control (CH:%d %d cents)",
			          ch, channel[ch].cc1.cutoff);
			break;
		case 0x2E: /* CC1 Amplitude Control */
			channel[ch].cc1.amp = (float)val / 64.0f - 1.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 Amplitude Control (CH:%d %.2f)", ch,
			          channel[ch].cc1.amp);
			break;
		case 0x2F: /* CC1 LFO1 Rate Control */
			channel[ch].cc1.lfo1_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO1 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].cc1.lfo1_rate);
			break;
		case 0x30: /* CC1 LFO1 Pitch Depth */
			channel[ch].cc1.lfo1_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO1 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].cc1.lfo1_pitch_depth);
			break;
		case 0x31: /* CC1 LFO1 Filter Depth */
			channel[ch].cc1.lfo1_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO1 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].cc1.lfo1_tvf_depth);
			break;
		case 0x32: /* CC1 LFO1 Amplitude Depth */
			channel[ch].cc1.lfo1_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO1 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].cc1.lfo1_tva_depth);
			break;
		case 0x33: /* CC1 LFO2 Rate Control */
			channel[ch].cc1.lfo2_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO2 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].cc1.lfo2_rate);
			break;
		case 0x34: /* CC1 LFO2 Pitch Depth */
			channel[ch].cc1.lfo2_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO2 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].cc1.lfo2_pitch_depth);
			break;
		case 0x35: /* CC1 LFO2 Filter Depth */
			channel[ch].cc1.lfo2_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO2 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].cc1.lfo2_tvf_depth);
			break;
		case 0x36: /* CC1 LFO2 Amplitude Depth */
			channel[ch].cc1.lfo2_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC1 LFO2 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].cc1.lfo2_tva_depth);
			break;
		case 0x37: /* CC2 Pitch Control */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 Pitch Control (CH:%d %d semitones)", ch,
			          channel[ch].cc2.pitch);
			break;
		case 0x38: /* CC2 Filter Cutoff Control */
			channel[ch].cc2.cutoff = (val - 64) * 150;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 Filter Cutoff Control (CH:%d %d cents)",
			          ch, channel[ch].cc2.cutoff);
			break;
		case 0x39: /* CC2 Amplitude Control */
			channel[ch].cc2.amp = (float)val / 64.0f - 1.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 Amplitude Control (CH:%d %.2f)", ch,
			          channel[ch].cc2.amp);
			break;
		case 0x3A: /* CC2 LFO1 Rate Control */
			channel[ch].cc2.lfo1_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO1 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].cc2.lfo1_rate);
			break;
		case 0x3B: /* CC2 LFO1 Pitch Depth */
			channel[ch].cc2.lfo1_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO1 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].cc2.lfo1_pitch_depth);
			break;
		case 0x3C: /* CC2 LFO1 Filter Depth */
			channel[ch].cc2.lfo1_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO1 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].cc2.lfo1_tvf_depth);
			break;
		case 0x3D: /* CC2 LFO1 Amplitude Depth */
			channel[ch].cc2.lfo1_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO1 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].cc2.lfo1_tva_depth);
			break;
		case 0x3E: /* CC2 LFO2 Rate Control */
			channel[ch].cc2.lfo2_rate = (float)(val - 64) / 6.4f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO2 Rate Control (CH:%d %.1f Hz)", ch,
			          channel[ch].cc2.lfo2_rate);
			break;
		case 0x3F: /* CC2 LFO2 Pitch Depth */
			channel[ch].cc2.lfo2_pitch_depth =
			    conv_lfo_pitch_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO2 Pitch Depth (CH:%d %d cents)", ch,
			          channel[ch].cc2.lfo2_pitch_depth);
			break;
		case 0x40: /* CC2 LFO2 Filter Depth */
			channel[ch].cc2.lfo2_tvf_depth =
			    conv_lfo_filter_depth(val);
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO2 Filter Depth (CH:%d %d cents)", ch,
			          channel[ch].cc2.lfo2_tvf_depth);
			break;
		case 0x41: /* CC2 LFO2 Amplitude Depth */
			channel[ch].cc2.lfo2_tva_depth = (float)val / 127.0f;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "CC2 LFO2 Amplitude Depth (CH:%d %.2f)", ch,
			          channel[ch].cc2.lfo2_tva_depth);
			break;
		case 0x42: /* Note Limit Low */
			channel[ch].note_limit_low = val;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Note Limit Low (CH:%d VAL:%d)", ch, val);
			break;
		case 0x43: /* Note Limit High */
			channel[ch].note_limit_high = val;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Note Limit High (CH:%d VAL:%d)", ch, val);
			break;
		case 0x44: /* Velocity Limit Low */
			channel[ch].vel_limit_low = val;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Velocity Limit Low (CH:%d VAL:%d)", ch, val);
			break;
		case 0x45: /* Velocity Limit High */
			channel[ch].vel_limit_high = val;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Velocity Limit High (CH:%d VAL:%d)", ch,
			          val);
			break;
		case 0x46: /* Rx. Note Off */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Drum Instrument Rx. Note Off (CH:%d NOTE:%d "
			          "VAL:%d)",
			          ch, note, val);
			break;
		case 0x47: /* Rx. Note On */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Drum Instrument Rx. Note On (CH:%d NOTE:%d "
			          "VAL:%d)",
			          ch, note, val);
			break;
		...
		case 0x60: /* Reverb Type (GM2) */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Reverb Type (%d)",
			          val);
		...
		case 0x61: /* Chorus Type (GM2) */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Type (%d)",
			          val);
		...
		channel[ch].sysex_gs_msb_addr = channel[ch].sysex_gs_msb_val =
		    0;
		switch (b) {
		case 0x00: /* EQ ON/OFF */
			if (!opt_eq_control) {
		...
		case 0x01: /* EQ LOW FREQ */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ LOW FREQ (%d)",
			          val);
			break;
		case 0x02: /* EQ LOW GAIN */
			...
			recompute_eq_status_gs();
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ LOW GAIN (%d dB)",
			          val - 0x40);
			break;
		case 0x03: /* EQ HIGH FREQ */
			...
			recompute_eq_status_gs();
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ HIGH FREQ (%d)",
			          val);
			break;
		case 0x04: /* EQ HIGH GAIN */
			...
			recompute_eq_status_gs();
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ HIGH GAIN (%d dB)",
			          val - 0x40);
			break;
		case 0x05: /* Reverb Macro */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Reverb Macro (%d)",
			          val);
		...
		case 0x07: /* Reverb Pre-LPF */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Reverb Pre-LPF (%d)",
			          val);
		...
		case 0x08: /* Reverb Level */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Reverb Level (%d)",
			          val);
		...
		case 0x09: /* Reverb Time */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Reverb Time (%d)",
			          val);
		...
		case 0x0D: /* Chorus Macro */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Macro (%d)",
			          val);
			set_chorus_macro_gs(val);
			recompute_chorus_status_gs();
			init_ch_chorus();
		...
		case 0x0E: /* Chorus Pre-LPF */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Pre-LPF (%d)",
			          val);
		...
		case 0x0F: /* Chorus Level */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Level (%d)",
			          val);
		...
		case 0x10: /* Chorus Feedback */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Feedback (%d)",
			          val);
		...
		case 0x11: /* Chorus Delay */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Delay (%d)",
			          val);
		...
		case 0x12: /* Chorus Rate */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Rate (%d)",
			          val);
		...
		case 0x13: /* Chorus Depth */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Depth (%d)",
			          val);
		...
		case 0x14: /* Chorus Send Level to Reverb */
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Chorus Send Level to Reverb (%d)", val);
		...
		case 0x15: /* Chorus Send Level to Delay */
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Chorus Send Level to Delay (%d)", val);
		...
		case 0x16: /* Delay Macro */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Delay Macro (%d)",
			          val);
		...
		case 0x17: /* Delay Pre-LPF */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Delay Pre-LPF (%d)",
			          val);
			val &= 0x7;
		...
		case 0x1E: /* Delay Level */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Delay Level (%d)",
			          val);
		...
		case 0x1F: /* Delay Feedback */
			ctl->cmsg(CMSG_INFO, VERB_NOISY, "Delay Feedback (%d)",
			          val);
		...
		case 0x20: /* Delay Send Level to Reverb */
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Delay Send Level to Reverb (%d)", val);
		...
		case 0x21: /* Velocity Sense Depth */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Velocity Sense Depth (CH:%d VAL:%d)", ch,
			          val);
			break;
		case 0x22: /* Velocity Sense Offset */
			channel[ch].velocity_sense_offset = val;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Velocity Sense Offset (CH:%d VAL:%d)", ch,
			          val);
			break;
		...
		case 0x24: /* Assign Mode */
			...
			if (val == 0) {
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Assign Mode: Single (CH:%d)", ch);
			} else if (val == 1) {
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Assign Mode: Limited-Multi (CH:%d)",
				          ch);
			} else if (val == 2) {
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Assign Mode: Full-Multi (CH:%d)",
				          ch);
			}
			break;
		case 0x25: /* TONE MAP-0 NUMBER */
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Tone Map-0 Number (CH:%d VAL:%d)", ch, val);
			break;
		case 0x26: /* Pitch Offset Fine */
			channel[ch].pitch_offset_fine =
			    (FLOAT_T)((((int32)val << 4) | (int32)val) - 0x80) /
			    10.0;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Pitch Offset Fine (CH:%d %3fHz)", ch,
			          channel[ch].pitch_offset_fine);
			break;
		case 0x27: /* Insertion Effect Parameter */
			...
			insertion_effect_gs.type =
			    ((int32)insertion_effect_gs.type_msb << 8) |
			    (int32)insertion_effect_gs.type_lsb;
		...
		case 0x28: /* Insertion Effect Parameter */
			...
			if (temp == insertion_effect_gs.type) {
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "EFX TYPE (%02X %02X)",
				          insertion_effect_gs.type_msb,
				          insertion_effect_gs.type_lsb);
				realloc_insertion_effect_gs();
			}
			break;
		case 0x29:
		...
		if (note == 3 && msb == 0) { /* Effect 2 */
			...
			case 0x00: /* Insertion Effect Type MSB */
				if (insertion_effect_xg[note].type_msb != val) {
					ctl->cmsg(CMSG_INFO, VERB_NOISY,
					          ...
					          "(%d %02X)",
					          note, val);
					insertion_effect_xg[note].type_msb =
					    val;
					realloc_effect_xg(
					    &insertion_effect_xg[note]);
				}
				break;
			case 0x01: /* Insertion Effect Type LSB */
				if (insertion_effect_xg[note].type_lsb != val) {
					ctl->cmsg(CMSG_INFO, VERB_NOISY,
					          ...
					          "(%d %02X)",
					          note, val);
					insertion_effect_xg[note].type_lsb =
					    val;
					realloc_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x0B:
				...
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "Insertion Effect Parameter %d (%d %d)",
				    temp + 1, note, val);
				if (insertion_effect_xg[note].param_lsb[temp] !=
				    val) {
					insertion_effect_xg[note]
					    .param_lsb[temp] = val;
					recompute_effect_xg(
			...
			case 0x0C: /* Insertion Effect Part */
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Insertion Effect Part (%d %d)", note,
				          val);
			...
			case 0x0D: /* MW Insertion Control Depth */
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "MW Insertion Control Depth (%d %d)",
				          note, val);
				if (insertion_effect_xg[note].mw_depth != val) {
					insertion_effect_xg[note].mw_depth =
					    val;
					recompute_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x0E: /* BEND Insertion Control Depth */
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "BEND Insertion Control Depth (%d %d)",
				    note, val);
				if (insertion_effect_xg[note].bend_depth !=
				    val) {
					insertion_effect_xg[note].bend_depth =
					    val;
					recompute_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x0F: /* CAT Insertion Control Depth */
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "CAT Insertion Control Depth (%d %d)",
				          note, val);
				if (insertion_effect_xg[note].cat_depth !=
				    val) {
					insertion_effect_xg[note].cat_depth =
					    val;
					recompute_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x10: /* AC1 Insertion Control Depth */
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "AC1 Insertion Control Depth (%d %d)",
				          note, val);
				if (insertion_effect_xg[note].ac1_depth !=
				    val) {
					insertion_effect_xg[note].ac1_depth =
					    val;
					recompute_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x11: /* AC2 Insertion Control Depth */
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "AC2 Insertion Control Depth (%d %d)",
				          note, val);
				if (insertion_effect_xg[note].ac2_depth !=
				    val) {
					insertion_effect_xg[note].ac2_depth =
					    val;
					recompute_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x12: /* CBC1 Insertion Control Depth */
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "CBC1 Insertion Control Depth (%d %d)",
				    note, val);
				if (insertion_effect_xg[note].cbc1_depth !=
				    val) {
					insertion_effect_xg[note].cbc1_depth =
					    val;
					recompute_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x13: /* CBC2 Insertion Control Depth */
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "CBC2 Insertion Control Depth (%d %d)",
				    note, val);
				if (insertion_effect_xg[note].cbc2_depth !=
				    val) {
					insertion_effect_xg[note].cbc2_depth =
					    val;
					recompute_effect_xg(
					    &insertion_effect_xg[note]);
				}
			...
			case 0x42:
				...
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "Insertion Effect Parameter %d MSB (%d %d)",
				    temp + 1, note, val);
				if (insertion_effect_xg[note].param_msb[temp] !=
				    val) {
					insertion_effect_xg[note]
					    .param_msb[temp] = val;
					recompute_effect_xg(
			...
			case 0x43:
				...
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "Insertion Effect Parameter %d LSB (%d %d)",
				    temp + 1, note, val);
				if (insertion_effect_xg[note].param_lsb[temp] !=
				    val) {
					insertion_effect_xg[note]
			...
			case 0x0B:
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Reverb Parameter %d (%d)",
				          b - 0x02 + 1, val);
				if (reverb_status_xg.param_lsb[b - 0x02] !=
				    val) {
					reverb_status_xg.param_lsb[b - 0x02] =
					    val;
			...
			case 0x2B:
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Chorus Parameter %d (%d)",
				          b - 0x22 + 1, val);
				if (chorus_status_xg.param_lsb[b - 0x22] !=
				    val) {
					chorus_status_xg.param_lsb[b - 0x22] =
					    val;
			...
			case 0x40: /* Variation Type MSB */
				if (note >= XG_VARIATION_EFFECT_NUM ||
				    note < 0) {
					break;
				}
				if (variation_effect_xg[note].type_msb != val) {
					ctl->cmsg(CMSG_INFO, VERB_NOISY,
					          "Variation Type MSB (%02X)",
					          val);
					variation_effect_xg[note].type_msb =
					    val;
					realloc_effect_xg(
					    &variation_effect_xg[note]);
				}
				break;
			case 0x41: /* Variation Type LSB */
				...
				if (variation_effect_xg[note].type_lsb != val) {
					ctl->cmsg(CMSG_INFO, VERB_NOISY,
					          "Variation Type LSB (%02X)",
					          val);
					variation_effect_xg[note].type_lsb =
					    val;
					realloc_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x54:
				...
				temp = (b - 0x42) / 2;
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Variation Parameter %d MSB (%d)",
				          temp, val);
				if (variation_effect_xg[note].param_msb[temp] !=
				    val) {
					variation_effect_xg[note]
					    .param_msb[temp] = val;
					recompute_effect_xg(
			...
			case 0x55:
				...
				temp = (b - 0x43) / 2;
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Variation Parameter %d LSB (%d)",
				          temp, val);
				if (variation_effect_xg[note].param_lsb[temp] !=
				    val) {
					variation_effect_xg[note]
					    .param_lsb[temp] = val;
					recompute_effect_xg(
			...
			case 0x58: /* Send Variation To Reverb */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Send Variation To Reverb (%d)", val);
				if (variation_effect_xg[note].send_reverb !=
				    val) {
					variation_effect_xg[note].send_reverb =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x59: /* Send Variation To Chorus */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Send Variation To Chorus (%d)", val);
				if (variation_effect_xg[note].send_chorus !=
				    val) {
					variation_effect_xg[note].send_chorus =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x5A: /* Variation Connection */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Variation Connection (%d)", val);
				if (variation_effect_xg[note].connection !=
				    val) {
					variation_effect_xg[note].connection =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x5C: /* MW Variation Control Depth */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "MW Variation Control Depth (%d)",
				          val);
				if (variation_effect_xg[note].mw_depth != val) {
					variation_effect_xg[note].mw_depth =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x5D: /* BEND Variation Control Depth */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "BEND Variation Control Depth (%d)",
				          val);
				if (variation_effect_xg[note].bend_depth !=
				    val) {
					variation_effect_xg[note].bend_depth =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x5E: /* CAT Variation Control Depth */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "CAT Variation Control Depth (%d)",
				          val);
				if (variation_effect_xg[note].cat_depth !=
				    val) {
					variation_effect_xg[note].cat_depth =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x5F: /* AC1 Variation Control Depth */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "AC1 Variation Control Depth (%d)",
				          val);
				if (variation_effect_xg[note].ac1_depth !=
				    val) {
					variation_effect_xg[note].ac1_depth =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x60: /* AC2 Variation Control Depth */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "AC2 Variation Control Depth (%d)",
				          val);
				if (variation_effect_xg[note].ac2_depth !=
				    val) {
					variation_effect_xg[note].ac2_depth =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x61: /* CBC1 Variation Control Depth */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "CBC1 Variation Control Depth (%d)",
				          val);
				if (variation_effect_xg[note].cbc1_depth !=
				    val) {
					variation_effect_xg[note].cbc1_depth =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x62: /* CBC2 Variation Control Depth */
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "CBC2 Variation Control Depth (%d)",
				          val);
				if (variation_effect_xg[note].cbc2_depth !=
				    val) {
					variation_effect_xg[note].cbc2_depth =
					    val;
					recompute_effect_xg(
					    &variation_effect_xg[note]);
				}
			...
			case 0x99: /* Rcv CHANNEL, remapped from 0x04 */
				...
				if (val == 0x7f)
					...
					if ((ch < REDUCE_CHANNELS) !=
					    (val < REDUCE_CHANNELS)) {
						channel[ch].port_select =
						    ch < REDUCE_CHANNELS ? 1
						                         : 0;
					}
					if ((ch % REDUCE_CHANNELS) !=
					    (val % REDUCE_CHANNELS)) {
			...
			case 0x0E: /* EG Decay1 */
				if (channel[ch].drums[note] == NULL)
					play_midi_setup_drums(ch, note);
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Drum Instrument EG Decay1 (CH:%d "
				          "NOTE:%d VAL:%d)",
				          ch, note, val);
				channel[ch]
				    .drums[note]
				    ->drum_envelope_rate[EG_DECAY1] = val;
				break;
			case 0x0F: /* EG Decay2 */
				if (channel[ch].drums[note] == NULL)
					play_midi_setup_drums(ch, note);
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Drum Instrument EG Decay2 (CH:%d "
				          "NOTE:%d VAL:%d)",
				          ch, note, val);
				channel[ch]
				    .drums[note]
				    ->drum_envelope_rate[EG_DECAY2] = val;
				break;
			default:
				break;
			}
...
static void play_midi_prescan(MidiEvent *ev) {
			...
			for (k = offset; k < offset + 16; k++) {
				...
				if (layered) {
					if (!IS_SET_CHANNELMASK(
					        channel[k].channel_layer,
					        port_ch) ||
					    channel[k].port_select !=
					        (orig_ch >> 4))
						continue;
					ev->channel = k;
				}
				...
				case ME_NOTEOFF:
					resamp_cache_refer_off(
					    ch, MIDI_EVENT_NOTE(ev), ev->time);
					break;
				...
				case ME_DRUMPART:
					if (midi_drumpart_change(ch, ev->a))
						midi_program_change(
						    ch, channel[ch].program);
					break;

				case ME_KEYSHIFT:
					resamp_cache_refer_alloff(ch, ev->time);
					channel[ch].key_shift =
					    (int)ev->a - 0x40;
					break;
	...
	if (0 < mainvolume_max && mainvolume_max < 0x7f) {
		compensation_ratio =
		    pow((double)0x7f / (double)mainvolume_max, 4);
		ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Compensation ratio:%lf",
		          compensation_ratio);
	}
...
static int last_rpn_addr(int ch) {
	...
	static struct rpn_tag_map_t nrpn_addr_map[] = {
	    {0x0108, 0xffff, NRPN_ADDR_0108}, {0x0109, 0xffff, NRPN_ADDR_0109},
	    {0x010a, 0xffff, NRPN_ADDR_010A}, {0x0120, 0xffff, NRPN_ADDR_0120},
	    {0x0121, 0xffff, NRPN_ADDR_0121}, {0x0130, 0xffff, NRPN_ADDR_0130},
	    {0x0131, 0xffff, NRPN_ADDR_0131}, {0x0134, 0xffff, NRPN_ADDR_0134},
	    {0x0135, 0xffff, NRPN_ADDR_0135}, {0x0163, 0xffff, NRPN_ADDR_0163},
	    {0x0164, 0xffff, NRPN_ADDR_0164}, {0x0166, 0xffff, NRPN_ADDR_0166},
	    {0x1400, 0xff00, NRPN_ADDR_1400}, {0x1500, 0xff00, NRPN_ADDR_1500},
	    {0x1600, 0xff00, NRPN_ADDR_1600}, {0x1700, 0xff00, NRPN_ADDR_1700},
	    {0x1800, 0xff00, NRPN_ADDR_1800}, {0x1900, 0xff00, NRPN_ADDR_1900},
	    {0x1a00, 0xff00, NRPN_ADDR_1A00}, {0x1c00, 0xff00, NRPN_ADDR_1C00},
	    {0x1d00, 0xff00, NRPN_ADDR_1D00}, {0x1e00, 0xff00, NRPN_ADDR_1E00},
	    {0x1f00, 0xff00, NRPN_ADDR_1F00}, {0x3000, 0xff00, NRPN_ADDR_3000},
	    {0x3100, 0xff00, NRPN_ADDR_3100}, {0x3400, 0xff00, NRPN_ADDR_3400},
	    {0x3500, 0xff00, NRPN_ADDR_3500}, {-1, -1, 0}};
...
static void update_rpn_map(int ch, int addr, int update_now) {
	...
	case NRPN_ADDR_0121: /* Filter Resonance */
		if (opt_lpf_def) {
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Filter Resonance (CH:%d VAL:%d)", ch,
			          val - 64);
	...
	case NRPN_ADDR_0130: /* EQ BASS */
		if (opt_eq_control) {
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "EQ BASS (CH:%d %.2f dB)", ch,
			          0.19 * (double)(val - 0x40));
	...
	case NRPN_ADDR_0134: /* EQ BASS frequency */
		if (opt_eq_control) {
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "EQ BASS frequency (CH:%d %d Hz)", ch,
			          (int32)eq_freq_table_xg[val]);
	...
	case NRPN_ADDR_0135: /* EQ TREBLE frequency */
		if (opt_eq_control) {
			...
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "EQ TREBLE frequency (CH:%d %d Hz)", ch,
			          (int32)eq_freq_table_xg[val]);
	...
	case NRPN_ADDR_1400: /* Drum Filter Cutoff (XG) */
		...
		ctl->cmsg(
		    CMSG_INFO, VERB_NOISY,
		    "Drum Instrument Filter Cutoff (CH:%d NOTE:%d VAL:%d)", ch,
		    note, val);
	...
	case NRPN_ADDR_1600: /* Drum EG Attack Time (XG) */
		...
		if (opt_tva_attack) {
			...
			val -= 64;
			ctl->cmsg(CMSG_INFO, VERB_NOISY,
			          "Drum Instrument Attack Time (CH:%d NOTE:%d "
			          "VAL:%d)",
			          ch, note, val);
			channel[ch].drums[note]->drum_envelope_rate[EG_ATTACK] =
			    val;
		}
		break;
	...
	case RPN_ADDR_0005: /* GM2: Modulation Depth Range */
		channel[ch].mod.lfo1_pitch_depth =
		    (((int32)channel[ch].rpnmap[RPN_ADDR_0005] << 7) |
		     channel[ch].rpnmap_lsb[RPN_ADDR_0005]) *
		    100 / 128;
		ctl->cmsg(CMSG_INFO, VERB_NOISY,
		          "Modulation Depth Range (CH:%d VALUE:%d)", ch,
		          channel[ch].rpnmap[RPN_ADDR_0005]);
		break;
		...
		for (j = 0; j < MAX_CHANNELS; j += 16) {
			...
			for (k = offset; k < offset + 16; k++) {
				...
				case ME_MASTER_VOLUME:
					master_volume_ratio =
					    (int32)current_event->a +
					    256 * (int32)current_event->b;
					break;
				...
				case ME_EXPRESSION:
					channel[ch].expression =
					    current_event->a;
					break;

				case ME_PROGRAM:
					midi_program_change(ch,
					                    current_event->a);
					break;
				...
				case ME_PORTAMENTO_TIME_MSB:
					channel[ch].portamento_time_msb =
					    current_event->a;
					break;

				case ME_PORTAMENTO_TIME_LSB:
					channel[ch].portamento_time_lsb =
					    current_event->a;
					break;
				...
				case ME_SOFT_PEDAL:
					if (opt_lpf_def) {
						channel[ch].soft_pedal =
						    current_event->a;
						ctl->cmsg(
						    CMSG_INFO, VERB_NOISY,
						    "Soft Pedal (CH:%d VAL:%d)",
						    ch, channel[ch].soft_pedal);
					}
					break;

				case ME_HARMONIC_CONTENT:
					if (opt_lpf_def) {
						channel[ch].param_resonance =
						    current_event->a - 64;
						ctl->cmsg(CMSG_INFO, VERB_NOISY,
						          "Harmonic Content "
						          "(CH:%d VAL:%d)",
				...
				case ME_BRIGHTNESS:
					if (opt_lpf_def) {
						channel[ch].param_cutoff_freq =
						    current_event->a - 64;
						ctl->cmsg(
						    CMSG_INFO, VERB_NOISY,
						    "Brightness (CH:%d VAL:%d)",
						    ch,
						    channel[ch]
						        .param_cutoff_freq);
					}
				...
				case ME_ATTACK_TIME:
					...
					set_envelope_time(ch, current_event->a,
					                  EG_ATTACK);
					break;
				...
				case ME_TEMPO:
					current_play_tempo =
					    ch + current_event->b * 256 +
					    current_event->a * 65536;
					break;
				...
				case ME_SHERRY:
					wrd_sherry_event(
					    ch | (current_event->a << 8) |
					    (current_event->b << 16));
					break;

				case ME_DRUMPART:
					if (midi_drumpart_change(
					        ch, current_event->a))
						midi_program_change(
						    ch, channel[ch].program);
					break;
				...
				case ME_KEYSHIFT:
					channel[ch].key_shift =
					    (int)current_event->a - 0x40;
					break;

				case ME_KEYSIG:
					if (opt_init_keysig != 8)
						break;
					current_keysig = current_event->a +
					                 current_event->b * 16;
					break;
				...
				case ME_SCALE_TUNING:
					channel[ch]
					    .scale_tuning[current_event->a] =
					    current_event->b;
					break;

				case ME_BULK_TUNING_DUMP:
					set_single_note_tuning(
					    ch, current_event->a,
					    current_event->b, 0);
					break;
				...
				case ME_TEMPER_KEYSIG:
					current_temper_keysig =
					    (current_event->a + 8) % 32 - 8;
					temper_adj =
					    ((current_event->a + 8) & 0x20) ? 1
					                                    : 0;
					break;
				...
				case ME_MASTER_TEMPER_TYPE:
					for (i = 0; i < MAX_CHANNELS; i++)
						channel[i].temper_type =
						    current_event->a;
					break;

				case ME_USER_TEMPER_ENTRY:
					set_user_temper_entry(ch,
					                      current_event->a,
					                      current_event->b);
					break;

				case ME_SYSEX_LSB:
					process_sysex_event(ME_SYSEX_LSB, ch,
					                    current_event->a,
					                    current_event->b);
					break;

				case ME_SYSEX_MSB:
					process_sysex_event(ME_SYSEX_MSB, ch,
					                    current_event->a,
					                    current_event->b);
					break;

				case ME_SYSEX_GS_LSB:
					process_sysex_event(ME_SYSEX_GS_LSB, ch,
					                    current_event->a,
					                    current_event->b);
					break;

				case ME_SYSEX_GS_MSB:
					process_sysex_event(ME_SYSEX_GS_MSB, ch,
					                    current_event->a,
					                    current_event->b);
					break;

				case ME_SYSEX_XG_LSB:
					process_sysex_event(ME_SYSEX_XG_LSB, ch,
					                    current_event->a,
					                    current_event->b);
					break;

				case ME_SYSEX_XG_MSB:
					process_sysex_event(ME_SYSEX_XG_MSB, ch,
					                    current_event->a,
					                    current_event->b);
					break;
...
static int playmidi_change_rate(int32 rate, int restart) {
	...
	if (rate < MIN_OUTPUT_RATE || rate > MAX_OUTPUT_RATE) {
		ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Out of sample rate: %d",
		          rate);
		return -1;
	}
...
int compute_freq_table_index(int current_keysig, int note_key_offset) {
	...
	while (i != 7) {
		assert(
		    j <=
		    11); /* circle of fifths always converges within 11 steps */
		i += i < 7 ? 5 : -7;
		j++;
	}
...
static int apply_controls(void) {
	...
	do {
		...
		case RC_KEYDOWN:
			note_key_offset += val;
			current_freq_table =
			    normalize_mod12(current_freq_table + val);
			current_temper_freq_table =
			    normalize_mod12(current_temper_freq_table + val);
			if (sync_restart(1) != -1)
				jump_flag = 1;
			ctl_mode_event(CTLE_KEY_OFFSET, 0, note_key_offset, 0);
			continue;
		...
		case RC_TOGGLE_DRUMCHAN:
			...
			SET_CHANNELMASK(current_file_info->drumchannel_mask,
			                val);
			if (IS_SET_CHANNELMASK(drumchannels, val)) {
				UNSET_CHANNELMASK(drumchannels, val);
				UNSET_CHANNELMASK(
				    current_file_info->drumchannels, val);
			} else {
				SET_CHANNELMASK(drumchannels, val);
				SET_CHANNELMASK(current_file_info->drumchannels,
				                val);
			}
			aq_flush(1);
			return RC_RELOAD;
		...
		case RC_TOGGLE_MUTE:
			...
			ctl_mode_event(
			    CTLE_MUTE, 0, val,
			    (IS_SET_CHANNELMASK(channel_mute, val)) ? 1 : 0);
			continue;
...
static void do_compute_data_midi(int32 count) {
	...
	int channel_effect, channel_reverb, channel_chorus, channel_delay,
	    channel_eq;
	...
	channel_reverb =
	    (stereo && (opt_reverb_control == 1 || opt_reverb_control == 3 ||
	...
	/* is EQ valid? */
	channel_eq = opt_eq_control && (eq_status_gs.low_gain != 0x40 ||
	                                eq_status_gs.high_gain != 0x40 ||
	                                play_system_mode == XG_SYSTEM_MODE);

	channel_effect =
	    (stereo && (channel_reverb || channel_chorus || channel_delay ||
	                channel_eq || opt_insertion_effect));

	uv = upper_voices;
	for (i = 0; i < uv; i++) {
		if (voice[i].status != VOICE_FREE) {
			channel[voice[i].channel].lasttime =
	...
	if (channel_effect) {
		...
		if (reverb_buffer ==
		    NULL) { /* allocating buffer for channel effect */
			reverb_buffer = (char *)safe_malloc(
			    MAX_CHANNELS * AUDIO_BUFFER_SIZE * 8);
		}

		for (i = 0; i < MAX_CHANNELS; i++) {
			if (opt_insertion_effect &&
			    channel[i].insertion_effect) {
				vpblist[i] = insertion_effect_buffer;
			} else if (channel[i].eq_gs ||
			           (get_reverb_level(i) !=
			                DEFAULT_REVERB_SEND_LEVEL &&
			...
			if (opt_drum_effect && ISDRUMCHANNEL(i)) {
				make_drum_effect(i);
				for (j = 0; j < channel[i].drum_effect_num;
				     j++) {
					if (channel[i].drum_effect[j].buf !=
					    NULL) {
						memset(channel[i]
						           .drum_effect[j]
						           .buf,
	...
	for (i = 0; i < uv; i++) {
		if (voice[i].status != VOICE_FREE) {
			...
			if (!IS_SET_CHANNELMASK(channel_mute,
			                        voice[i].channel)) {
	...
	if (play_system_mode == XG_SYSTEM_MODE && channel_effect) { /* XG */
		if (opt_insertion_effect) { /* insertion effect */
			for (i = 0; i < XG_INSERTION_EFFECT_NUM; i++) {
				if (insertion_effect_xg[i].part <=
				    MAX_CHANNELS) {
					do_insertion_effect_xg(
					    vpblist[insertion_effect_xg[i]
					                .part],
					    cnt, &insertion_effect_xg[i]);
				}
			}
			for (i = 0; i < XG_VARIATION_EFFECT_NUM; i++) {
				if (variation_effect_xg[i].part <=
				    MAX_CHANNELS) {
					do_insertion_effect_xg(
					    vpblist[variation_effect_xg[i]
					                .part],
					    cnt, &variation_effect_xg[i]);
				}
			}
		}
		for (i = 0; i < MAX_CHANNELS; i++) { /* system effects */
			...
			if (p != buffer_pointer) {
				if (opt_drum_effect && ISDRUMCHANNEL(i)) {
					for (j = 0;
					     j < channel[i].drum_effect_num;
					     j++) {
						de = &(
						    channel[i].drum_effect[j]);
						if (de->reverb_send > 0) {
							set_ch_reverb(
							    de->buf, cnt,
							    de->reverb_send);
					...
					if (channel_chorus &&
						...
						set_ch_chorus(
						    p, cnt,
						    channel[i].chorus_level);
					}
					if (channel_delay &&
					    channel[i].delay_level > 0) {
						set_ch_delay(
						    p, cnt,
						    channel[i].delay_level);
					}
					if (channel_reverb &&
		...
		if (channel_reverb) {
			set_ch_reverb(buffer_pointer, cnt,
			              DEFAULT_REVERB_SEND_LEVEL);
		}
		...
		if (opt_insertion_effect) { /* insertion effect */
			...
			set_ch_chorus(insertion_effect_buffer, cnt,
			              insertion_effect_gs.send_chorus);
			set_ch_delay(insertion_effect_buffer, cnt,
			             insertion_effect_gs.send_delay);
			set_ch_reverb(insertion_effect_buffer, cnt,
			              insertion_effect_gs.send_reverb);
...
static void do_compute_data_midi(int32 count) {
	...
	channel_effect = (stereo && (opt_reverb_control || opt_chorus_control ||
	                             opt_delay_control || opt_eq_control ||
	                             opt_insertion_effect));
	uv = upper_voices;
	for (i = 0; i < uv; i++)
		if (voice[i].status != VOICE_FREE)
		...
		for (i = 0; i < MAX_CHANNELS; i++) {
			...
			if (channel[i].reverb_id != -1 &&
				...
				if (reverb_buffer == NULL)
					reverb_buffer = (char *)safe_malloc(
					    MAX_CHANNELS * AUDIO_BUFFER_SIZE *
					    8);
				if (channel[i].reverb_id != i)
					vpblist[i] =
					    vpblist[channel[i].reverb_id];
				else {
					vpblist[i] =
					    (int32 *)(reverb_buffer + chbufidx);
					chbufidx += n;
				}
			} else
				vpblist[i] = buffer_pointer;
...
static void do_compute_data_wav(int32 count) {
	...
	act_samples =
	    tf_read(wav_buffer, 1, req_size, current_file_info->pcm_tf) / 2;
	for (i = 0; i < act_samples; i++) {
		v = (uint16)LE_SHORT(wav_buffer[i]);
		buffer_pointer[i] =
		    (int32)((v << 16) | (v ^ 0x8000)) / 4; /* 4 : level down */
	}
	for (; i < samples; i++)
		buffer_pointer[i] = 0;
...
static int midi_play_end(void) {
	...
	if (opt_realtime_playing)
		rc = compute_data(
		    (int32)(play_mode->rate * PLAY_INTERLEAVE_SEC / 2));
	else
		rc = compute_data(
		    (int32)(play_mode->rate * PLAY_INTERLEAVE_SEC));
	if (RC_IS_SKIP_FILE(rc))
		goto midi_end;
...
static int compute_data(int32 count) {
	...
	while ((count + buffered_count) >= audio_buffer_size) {
		...
		ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY, "output data (%d)",
		          audio_buffer_size);
		...
		if (reduce_voice_threshold &&
		    (play_mode->flag & PF_CAN_TRACE) && !aq_fill_buffer_flag &&
		    aq_get_dev_queuesize() > 0) {
			/* Reduce voices if there is not enough audio device
			 * buffer */
			...
			} else                    /* Use default threshold */
			{
				...
				if (filled_limit >
				    maxfill / 5) /* too small audio buffer */
				{
					rate_limit -= 100 * audio_buffer_size /
					              maxfill / 5;
					filled_limit = 1;
				}
			}
			...
			/* The old method of calculating rate resulted in very
			   low values when using the new high order interplation
			   methods on "slow" CPUs when the queue was being
			   drained WAY too quickly.  This caused premature voice
			   reduction under Linux, even if the queue was over
			   2000%, leading to major voice lossage. */
			...
			if (!opt_realtime_playing) {
				...
				if (auto_reduce_polyphony && rate < 85) {
					 ...
					 */
					if (current_sample >= ok_nv_sample &&
					    ok_nv_counts > 1) {
						ok_nv_total >>= 1;
						ok_nv_counts >>= 1;
						ok_nv_sample =
			...
			/* EAW -- if buffer is < 75%, start reducing some voices
			   to try to let it recover.  This really helps a lot,
			   preserves decent sound, and decreases the frequency
			   of lost ON notes */
			if ((!opt_realtime_playing && rate < rate_limit) ||
				...
				if (filled <= last_filled) {
					...
					for (i = kill_nv = 0; i < upper_voices;
					     i++) {
						if (voice[i].status &
						        VOICE_FREE ||
						    voice[i].cache != NULL)
					...
					/* EAW -- buffer is dangerously low,
					   drasticly reduce voices to a
					   hopefully "safe" amount */
					if (filled < filled_limit &&
					    (opt_realtime_playing ||
					     rate < 10)) {
						FLOAT_T n;

						/* calculate the drastic voice
						 * reduction */
						if (nv >
						    kill_nv) /* Avoid division
						                by zero */
						{
							n = (FLOAT_T)nv /
							    (nv - kill_nv);
						...
						/* the buffer is still high
						   enough that we can throw
						   fewer voices away; keep the
						   ON voices, use the minimum
						   "bad" nv as a floor on voice
						   reductions */
					...
					/* lower max # of allowed voices to let
					 * the buffer recover */
					if (auto_reduce_polyphony) {
						temp_nv = nv - kill_nv;
						ok_nv =
						    ok_nv_total / ok_nv_counts;

						/* decrease it to current nv
						 * left */
						if (voices > temp_nv &&
						    temp_nv > ok_nv)
							voice_decrement_conservative(
							    voices - temp_nv);
						...
						/* increase the polyphony */
							...
							voice_increment(ok_nv -
							                voices);
					}

					while (upper_voices > 0 &&
					       voice[upper_voices - 1].status ==
					           VOICE_FREE)
						upper_voices--;
				}
				last_filled = filled;
				...
				if (!opt_realtime_playing &&
					...
					if (auto_reduce_polyphony) {
						/* reset ok_nv stuff when out of
						 * danger */
						ok_nv_total =
						    max_good_nv * ok_nv_counts;
						if (ok_nv_counts > 1) {
							ok_nv_total >>= 1;
							ok_nv_counts >>= 1;
						}

						/* restore max # of allowed
						 * voices to normal */
						restore_voices(0);
					}
				}
		...
		if (upper_voices == 0 && check_eot_flag &&
		    (i = check_midi_play_end(current_event,
		                             EOT_PRESEARCH_LEN)) > 0) {
			if (i > 1)
				ctl->cmsg(CMSG_INFO, VERB_VERBOSE,
				          "Last %d MIDI events are ignored",
				          i - 1);
...
static void update_modulation_wheel(int ch) {
	...
	for (i = 0; i < uv; i++)
		if (voice[i].status != VOICE_FREE && voice[i].channel == ch) {
			/* Set/Reset mod-wheel */
			voice[i].vibrato_control_counter =
			    voice[i].vibrato_phase = 0;
			recompute_amp(i);
			apply_envelope_to_amp(i);
			recompute_freq(i);
			recompute_voice_filter(i);
...
static void update_portamento_controls(int ch) {
	if (!channel[ch].portamento || (channel[ch].portamento_time_msb |
	                                channel[ch].portamento_time_lsb) == 0)
		drop_portamento(ch);
	else {
		double mt, dc;
...
int play_event(MidiEvent *ev) {
	...
	if (cet > current_sample) {
		...
		if (midi_streaming != 0)
			if ((cet - current_sample) * 1000 / play_mode->rate >
				...
				/* reset_voices(); */
				/* ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY,
				                "play_event: discard %d
				   samples", cet - current_sample); */
				current_sample = cet;
			}
#endif
	...
	for (k = 0; k < MAX_CHANNELS; k += 16) {
		...
		for (l = offset; l < offset + 16; l++) {
			...
			if (layered) {
				if (!IS_SET_CHANNELMASK(
				        channel[l].channel_layer, port_ch) ||
			...
			case ME_PITCHWHEEL:
				...
				adjust_pitch(ch);
				ctl_mode_event(CTLE_PITCH_BEND, 1, ch,
				               channel[ch].pitchbend);
				break;
			...
			case ME_MODULATION_WHEEL:
				...
				ctl_mode_event(CTLE_MOD_WHEEL, 1, ch,
				               channel[ch].mod.val);
				break;
			...
			case ME_SUSTAIN:
				...
				if (channel[ch].sustain == 0 &&
				    channel[ch].sostenuto == 0)
					drop_sustain(ch);
				ctl_mode_event(CTLE_SUSTAIN, 1, ch,
				               channel[ch].sustain);
				break;
			...
			case ME_SOSTENUTO:
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY, "Sostenuto %d",
				          channel[ch].sostenuto);
				break;
			...
			case ME_LEGATO_FOOTSWITCH:
				...
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Legato Footswitch (CH:%d VAL:%d)",
				          ch, channel[ch].legato);
				break;

			case ME_HOLD2:
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "Hold2 - this function is not supported.");
				break;

			case ME_BREATH:
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "Breath - this function is not supported.");
				break;

			case ME_FOOT:
				ctl->cmsg(
				    CMSG_INFO, VERB_NOISY,
				    "Foot - this function is not supported.");
				break;

			case ME_BALANCE:
				ctl->cmsg(CMSG_INFO, VERB_NOISY,
				          "Balance - this function is not "
				          "supported.");
				break;
			...
			case ME_HARMONIC_CONTENT:
				if (opt_lpf_def) {
					...
					ctl->cmsg(
					    CMSG_INFO, VERB_NOISY,
					    "Harmonic Content (CH:%d VAL:%d)",
					    ch, channel[ch].param_resonance);
				}
				break;
			...
			case ME_REVERB_EFFECT:
				if (opt_reverb_control) {
					if (ISDRUMCHANNEL(ch) &&
					    get_reverb_level(ch) != ev->a) {
						channel[ch].drum_effect_flag =
						    0;
					}
					set_reverb_level(ch, ev->a);
					ctl_mode_event(CTLE_REVERB_EFFECT, 1,
					               ch,
					               get_reverb_level(ch));
				}
				break;
			...
			case ME_CHORUS_EFFECT:
				if (opt_chorus_control) {
					...
					ctl_mode_event(CTLE_CHORUS_EFFECT, 1,
					               ch,
					               get_chorus_level(ch));
					if (ev->a) {
						ctl->cmsg(CMSG_INFO, VERB_NOISY,
			...
			case ME_TEMPO:
				current_play_tempo =
				    ch + ev->b * 256 + ev->a * 65536;
				ctl_mode_event(CTLE_TEMPO, 1,
				               current_play_tempo, 0);
				break;
			...
			case ME_WRD:
				push_midi_trace2(wrd_midi_event, ch,
				                 current_event->a |
				                     (current_event->b << 8));
				break;
			...
			case ME_DRUMPART:
				if (midi_drumpart_change(ch,
				                         current_event->a)) {
					/* Update bank information */
					midi_program_change(
					    ch, channel[ch].program);
					ctl_mode_event(CTLE_DRUMPART, 1, ch,
					               ISDRUMCHANNEL(ch));
					ctl_prog_event(ch);
				}
				break;
			...
			case ME_KEYSIG:
				if (opt_init_keysig != 8)
					break;
				current_keysig =
				    current_event->a + current_event->b * 16;
				ctl_mode_event(CTLE_KEYSIG, 1, current_keysig,
				               0);
				if (opt_force_keysig != 8) {
					i = current_keysig -
					    ((current_keysig < 8) ? 0 : 16),
					j = 0;
					while (i != opt_force_keysig &&
					       i != opt_force_keysig + 12)
						i += (i > 0) ? -5 : 7, j++;
					while (abs(j - note_key_offset) > 7)
						j += (j > note_key_offset) ? -12
						                           : 12;
					...
					note_key_offset = j;
					kill_all_voices();
					ctl_mode_event(CTLE_KEY_OFFSET, 1,
					               note_key_offset, 0);
				}
				current_temper_freq_table =
				    compute_freq_table_index(
				        current_temper_keysig, note_key_offset);
				break;
			...
			case ME_SCALE_TUNING:
				resamp_cache_refer_alloff(ch,
				                          current_event->time);
				channel[ch].scale_tuning[current_event->a] =
				    current_event->b;
				adjust_pitch(ch);
				break;

			case ME_BULK_TUNING_DUMP:
				set_single_note_tuning(ch, current_event->a,
				                       current_event->b, 0);
				break;
			...
			case ME_TEMPER_KEYSIG:
				current_temper_keysig =
				    (current_event->a + 8) % 32 - 8;
				temper_adj =
				    ((current_event->a + 8) & 0x20) ? 1 : 0;
				ctl_mode_event(CTLE_TEMPER_KEYSIG, 1,
				               current_event->a, 0);
				current_temper_freq_table =
				    compute_freq_table_index(
				        current_temper_keysig, note_key_offset);
			...
			case ME_TEMPER_TYPE:
				...
				if (temper_type_mute) {
					if (temper_type_mute &
					    (1 << (current_event->a -
					           ((current_event->a >= 0x40)
					                ? 0x3c
					                : 0)))) {
						SET_CHANNELMASK(channel_mute,
						                ch);
						ctl_mode_event(CTLE_MUTE, 1, ch,
						               1);
					} else {
						UNSET_CHANNELMASK(channel_mute,
						                  ch);
						ctl_mode_event(CTLE_MUTE, 1, ch,
						               0);
					}
			...
			case ME_USER_TEMPER_ENTRY:
				set_user_temper_entry(ch, current_event->a,
				                      current_event->b);
				break;
			...
			case ME_CUEPOINT:
				set_cuepoint(ch, current_event->a,
				             current_event->b);
				break;
...
static int play_midi(MidiEvent *eventlist, int32 samples) {
	...
	reset_midi(0);
	if (!opt_realtime_playing && allocate_cache_size > 0 &&
	    !IS_CURRENT_MOD_FILE && (play_mode->flag & PF_PCM_STREAM)) {
...
static int load_pcm_file_wav() {
	...
	if (strcmp(pcm_alternate_file, "auto") == 0) {
		if (asprintf(&filename, "%s.wav", current_file_info->filename) <
		    0)
			return -1;
	} else if (strlen(pcm_alternate_file) >= 5 &&
	           strncasecmp(pcm_alternate_file + strlen(pcm_alternate_file) -
	                           4,
	                       ".wav", 4) == 0)
		filename = strdup(pcm_alternate_file);
	else
		return -1;
...
static int load_pcm_file_aiff() {
	...
	if (strcmp(pcm_alternate_file, "auto") == 0) {
		if (asprintf(&filename, "%s.aiff",
		             current_file_info->filename) < 0)
			return -1;
...
static int play_midi_load_file(char *fn, MidiEvent **event, int32 *nsamples) {
	int rc;
	...
	*event = read_midi_file(tf, &nevents, nsamples, fn);
	...
	ctl->cmsg(CMSG_INFO, VERB_NOISY,
	          "%d supported events, %d samples, time %d:%02d", nevents,
	          *nsamples, *nsamples / play_mode->rate / 60,
	...
	if (!IS_CURRENT_MOD_FILE && (play_mode->flag & PF_PCM_STREAM)) {
		/* FIXME: Instruments is not need for pcm_alternate_file. */

		/* Load instruments
		 * If opt_realtime_playing, the instruments will be loaded
		 * later.
		 */
		if (!opt_realtime_playing) {
			rc = RC_NONE;
...
int play_midi_file(char *fn) {
	...
	current_keysig = (int8)(opt_init_keysig == 8 ? 0 : opt_init_keysig);
...
static void ctl_timestamp(void) {
	CtlEvent ce;
	static int64 last_secs = -1, last_voices = -1;

	int64 secs =
	    (int64)(current_sample / (midi_time_ratio * play_mode->rate));

	int voices = 0;
	for (int i = 0; i < upper_voices; i++)
		if (voice[i].status != VOICE_FREE)
			voices++;
...
static void ctl_prog_event(int ch) {
	...
	ce.v4 =
	    (bank | (channel[ch].bank_lsb << 8) | (channel[ch].bank_msb << 16));
	if (ctl->trace_playing)
		push_midi_trace_ce(ctl->event, &ce);
	else
		ctl->event(&ce);
...
void playmidi_stream_init(void) {
	...
	COPY_CHANNELMASK(current_file_info->drumchannel_mask,
	                 default_drumchannel_mask);
	for (uint8 i = 0; i < MAX_CHANNELS; i++)
		for (uint8 j = 0; j < 128u; j++)
			channel[i].drums[j] = NULL;
	change_system_mode(DEFAULT_SYSTEM_MODE);
	reset_midi(0);
...
void playmidi_stream_free(void) { reuse_mblock(&playmidi_pool); }
...
static int16 get_midi_controller_pitch_depth(midi_controller *p) {
	return (int16)((float)p->val * (float)p->lfo1_pitch_depth *
	               (1.0f / 127.0f * 256.0 / 400.0));
}

static int16 get_midi_controller_amp_depth(midi_controller *p) {
	return (int16)((float)p->val * (float)p->lfo1_tva_depth *
	               (1.0f / 127.0f * 256.0));
}

static void init_rx(int ch) { channel[ch].rx = 0xFFFFFFFFu; /* all on */ }
...
void init_reverb_buffer(void) {
	if (reverb_buffer == NULL)
		reverb_buffer =
		    (char *)safe_malloc(MAX_CHANNELS * AUDIO_BUFFER_SIZE * 8u);
}
